...
def numDecodings(s):
    """
    91. 解码方法（review）
    s[i] 的状态：s[i] == 0 ，则s[i] 的方法数 = 0 ↓
                s[i] == 0 and s[i-1]  <= 2 , 则s[i] 的方法数 = s[i-2] 的方法数

                s[i] != 0 ，则s[i] 的方法数 = s[i-1] 的方法数↓
                s[i] == > 6 ，则s[i] 的方法数 = s[i-1] 的方法数 ↓
                s[i] == 1 — 6 and s[i - 1] != 0， 则s[i] 的方法数 = s[i-1] 的方法数 + s[i-2] 的方法数（s[i-1] 与 s[i-2] 是一个整体）
